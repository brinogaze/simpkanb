...
const DEFAULT_COLUMNS: Record<string, string[]> = {
  ...
  // General
  personal: ['Someday', 'Esta semana', 'Hoje', 'Feito'],
  gaming:   ['Wishlist', 'Em progresso', 'Farmando', 'Concluído'],
  work:     ['Backlog', 'A fazer', 'Em andamento', 'Revisão', 'Feito'],
  study:    ['Para estudar', 'Estudando', 'Revisar', 'Dominado'],
  other:    ['A fazer', 'Em andamento', 'Feito'],
}
...
export function createProject(data: Record<string, unknown>) {
  const db = getDb()
  const id = uuid()
  const projectType = (data.type as string) || 'personal'
  ...
  const cols = DEFAULT_COLUMNS[projectType] || DEFAULT_COLUMNS.other
